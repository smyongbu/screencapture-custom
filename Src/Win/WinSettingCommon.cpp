...
void WinSettingCommon::initTooltipDelayCtrls()
{
    ...
    label->setText(Lang::get(L"setting.tooltipDelay"));
    label->setHeightPercent(100.f);
    label->setJustifyContent(Ling::Justify::Center);
    label->setFlexGrow(1.f);

    tooltipDelayBtn = box->makeChild<Ling::Button>();
    tooltipDelayBtn->setText(std::format(L"{:.1f}\u79d2", Setting::get()->getTooltipDelay()));
    tooltipDelayBtn->setHeight(28.f);
    tooltipDelayBtn->setWidth(120.f);
    tooltipDelayBtn->setBorder(1.f, 0xE0E0E0FF);
    tooltipDelayBtn->setHoverBg(0Xf2f2f2FF);
    tooltipDelayBtn->onClick.add([this](Ling::Button* btn) {
        if (selectBox) return;
        showTooltipDelayBox(btn);
    });
...
void WinSettingCommon::initToolbarScaleCtrls()
{
    ...
    scaleBtn->setHoverBg(0Xf2f2f2FF);
...
void WinSettingCommon::initLangCtrls()
{
    ...
    selectBtn->setHoverBg(0Xf2f2f2FF);
...
void WinSettingCommon::showSelectBox(Ling::Button* btn)
{
    ...
    onMouseDownToken = win->onMouseDown.add([this,weakThis](POINT pos, bool isRight) {
        ...
        if (this->tooltipDelayBtn && this->tooltipDelayBtn->isPosIn(pos)) return;
...
void WinSettingCommon::showTooltipDelayBox(Ling::Button* btn)
{
    auto weakThis = getWeakThis();
    onMouseDownToken = win->onMouseDown.add([this, weakThis](POINT pos, bool isRight) {
        if (!weakThis.lock() || !selectBox) return;
        if (tooltipDelayBtn && tooltipDelayBtn->isPosIn(pos)) return;
        if (selectBox->isPosIn(pos)) return;
        hideSelectBox();
    });
    ...
    for (int i = 0; i < optionCount; ++i) {
        const auto delay = static_cast<float>(i) / 10.f;
        auto item = selectBox->makeChild<Ling::Button>();
        item->setText(std::format(L"{:.1f}秒", delay));
        item->setHeight(itemH);
        item->setWidthPercent(100.f);
        item->setHoverBg(0Xf2f2f2FF);
        item->setHoverColor(0X000000FF);
        item->onClick.add([this, delay](Ling::Button*) {
            Setting::get()->setTooltipDelay(delay);
            tooltipDelayBtn->setText(std::format(L"{:.1f}秒", delay));
            hideSelectBox();
        });
    }
}
